	...
	earPoints, TArray<int>& reflexIndices, TArray<int>& convexIndices, TArray<int>& earIndices) const
{
	...
	//Testing if point is an ear
	{
		for (int curIndex = 0; curIndex < convexIndices.Num(); curIndex++)
		{
			...
			const int prevIndex = CircularIndex(curElement - 1, splinePoints.Num());
			const int nextIndex = CircularIndex(curElement + 1, splinePoints.Num());
...
void ASplineArea::CreateAreaOutline()
{
	...
	for (int i = 0; i < instanceCount; i++)
	{
		...
		FVector secondPoint = pSpline->GetLocationAtSplinePoint(CircularIndex(i + 1, pSpline->GetNumberOfSplinePoints()), ESplineCoordinateSpace::World);
